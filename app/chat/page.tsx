...
function formatTime(date: Date) {
    // Ensure we parse standard ISO strings from DB or Date objects cleanly
...
export default function ChatPage() {
    ...
    // 1. Tracks the active chat conversation state parameters dynamically
    ...
    // 2. NEW: FETCH AND RESUME PREVIOUS CONSULTATION HISTORY LOGS IF SESSION ID IS PRESENT
    useEffect(() => {
        ...
        if (urlSessionId) {
            async function loadPastConsultation() {
                ...
                try {
                    ...
                    if (data && data.messages && data.messages.length > 0) {
                        // Cast string timestamps from the DB back into functional Date types
            ...
            // No sessionId parameters means flush the screen clear for a fresh thread instance
            setMessages([
                {
                    role: "assistant",
                    content: "Namaste 🌿 I am Vaidya, your Ayurvedic guide. Ask me about ancient home remedies, herbal nushkas, or any health concern — I'll share wisdom from the classical texts.",
    ...
    // Initialize Web Speech API safely on mount
    useEffect(() => {
    ...
    // Bind current target language configuration safely to recording instances
    useEffect(() => {
    ...
    async function sendMessage(override?: string) {
        ...
        try {
            ...
            // 3. UPDATED: TRANSMIT ACTIVE SESSION ID PARAMETERS BACKEND FOR APPENDING LOGS
            const res = await fetch("/api/chat", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ 
                    message: extendedMessagePayload,
                    sessionId: activeSessionId // 👈 Keeps database records bound cleanly to the same thread
                }),
            });
            const data = await res.json();
            ...
            // 4. If this message created a brand new session, lock the ID in without layout re-renders
      ...
      `}</style>
            <div className="vc-root">
                ...
                <header className="vc-header">
                    ...
                    {/* Integrated Selection Control Alignment */}
                    <div className="flex items-center gap-3" style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        ...
                        {/* Updated to link back cleanly to your profile dashboard path */}
